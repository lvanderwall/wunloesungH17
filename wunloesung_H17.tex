...
\lf Betrachte $L=0$, aus A1a) folgt \marginpar{\fbox{$L=0$}}$b_3=a_2=0,\quad b_2,\:a_1\neq0\quad\arr\quad\grad\uP(\us)-\grad\uQ(\us)=2-1=1>0$, also ist das NWM bzgl. $v(t)$ (mind.) einmal differenzierend! Vereinfache den FG und prüfe dann wieder auf Teilerfremdheit:
\[%
...
%
	\uP_4(\us_1)&=\frac{%
		R_2\bigl(-R_1C_1(R_1+\cancel{R_3})+R_1R_3(\cancel{C_1}+C_3)\bigr)\bigl(-R_3C_3(\cancel{R_1}+R_3)+R_1R_3(C_1+\cancel{C_3})\bigr)%
	}{%
...
Berechne die ÜF mit $R_i=R,\:C_i=C,\:L=0$ und dem NWM aus \Fref{a1d} in Admittanzen (Achtung: Für $\uY_{1,2}$ in Reihe git $\uY_{ges}=\uY_1\|\uY_2$). Verwende außerdem $(*)\quad\uY(\us):=\us C+\frac{1}{R}$:
\begin{align*}%
...
%
	=\ubcom{=:\BZVec_0(\us)}{%
		\LGSMat{c}{%
			-C_1u_{C_1,0}-\frac{i_{L_1,0}}{\us}+G_2\uV(\us)\\[2.5mm]%
...
%
	\utcom{Cramer}{=}\frac{%
		\det\LGSMat{ccc}{%
			\us C_1+G_1+G_3+\frac{1}{\us L_1} & G_2\cancel{\uV(\us)} & 0\\[2.5mm]%
		%
...
Berechne den FG mit (formaler) KWSR und einfachem Spannungsteiler in Impedanzen:
\begin{align*}%
